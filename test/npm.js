...
tap.test('npm packages - put pkg -> get file - scoped successfully uploaded', async (t) => {
    ...
    t.equals(uploaded.headers.get('location'), `${address}/npm/@cuz/fuzz/1.4.8`, 'on PUT of package, server should respond with a location header');
...
tap.test('npm packages - put pkg -> get file - non scoped successfully uploaded', async (t) => {
    const { headers, address } = t.context;
