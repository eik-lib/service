...
test("packages - no auth token on PUT - scoped", async () => {
	const formData = new FormData();
...
test("packages - no auth token on PUT - non scoped", async () => {
	const formData = new FormData();
	formData.append("package", new Blob([fs.readFileSync(FIXTURE_PKG)]));
...
test("packages - configure max size", async () => {
	const sink = new Sink();
	...
	try {
		...
		const res = await fetch(`${localAddress}/auth/login`, {
			method: "POST",
			body: loginFormData,
		});

		const { token } = /** @type {{ token: string }} */ (await res.json());
		const localHeaders = { Authorization: `Bearer ${token}` };

		const formData = new FormData();
		formData.append("package", new Blob([fs.readFileSync(FIXTURE_PKG)]));

		// PUT files on server
		const uploaded = await fetch(`${localAddress}/pkg/fuzz/8.4.1/`, {
			method: "PUT",
			body: formData,
