...
const EikService = class EikService {
    constructor({
        ...
        this._versionsGet = new http.VersionsGet({ organizations, sink: this.sink, logger: this.log });
        this._aliasPost = new http.AliasPost({ organizations, sink: this.sink, logger: this.log });
        this._aliasDel = new http.AliasDel({ organizations, sink: this.sink, logger: this.log });
        this._aliasGet = new http.AliasGet({ organizations, sink: this.sink, logger: this.log });
        this._aliasPut = new http.AliasPut({ organizations, sink: this.sink, logger: this.log });
        this._authPost = new http.AuthPost({ organizations, logger: this.log, authKey: config.get('basicAuth.key') });
        this._pkgLog = new http.PkgLog({ organizations, sink: this.sink, logger: this.log });
        this._pkgGet = new http.PkgGet({ organizations, sink: this.sink, logger: this.log });
        this._pkgPut = new http.PkgPut({ organizations, sink: this.sink, logger: this.log });
        this._mapGet = new http.MapGet({ organizations, sink: this.sink, logger: this.log });
        this._mapPut = new http.MapPut({ organizations, sink: this.sink, logger: this.log });
        ...
        if (config.get('sink.type') === 'fs') {
            this.log.info(`Server is running with the file system sink. Uploaded files will be stored under "${config.get('sink.path')}"`);
        }

        if (config.get('sink.type') === 'mem') {
            this.log.info(`Server is running with the in memory sink. Uploaded files will be lost on restart!`);
        }

        const hosts = config.get('organization.hostnames').join(', ');
        this.log.info(`Files for "${hosts}" will be stored in the "${config.get('organization.name')}" organization space`);
