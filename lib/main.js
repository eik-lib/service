...
const EikService = class EikService {
	...
	constructor(options = {}) {
		...
		const mergeStreams = (/** @type {any[]} */ ...streams) => {
			...
			// Each pipe() call adds several cleanup listeners (close, error,
			// finish, drain, unpipe) to str. Set the limit up front based on
			// the known number of sources so Node.js does not emit a
			// MaxListenersExceededWarning when the default of 10 is exceeded.
			str.setMaxListeners(str.getMaxListeners() + streams.length);
